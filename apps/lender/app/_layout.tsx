...
export default function RootLayout() {
  ...
  const checkAuth = async () => {
    try {
      const stored = await SecureStore.getItemAsync('dev_unlocked_lender');
      if (stored === 'true') {
        setIsUnlocked(true);
      }
    } catch (e) {
  ...
  const handleLogin = async () => {
    if (password === 'admin') {
      await SecureStore.setItemAsync('dev_unlocked_lender', 'true');
      setIsUnlocked(true);
      setError('');
    } else {
  ...
  if (!isUnlocked) {
    return (
      <View style={[styles.container, { justifyContent: 'center', padding: 24 }]}>
        <Text style={styles.title}>Lender Dashboard Gate</Text>
        <Text style={styles.subtitle}>Enter development password to manage Products & Criteria</Text>
        ...
        <TouchableOpacity style={styles.button} onPress={handleLogin}>
          <Text style={styles.buttonText}>Unlock LenderApp</Text>
        </TouchableOpacity>
      </View>
    );
  }

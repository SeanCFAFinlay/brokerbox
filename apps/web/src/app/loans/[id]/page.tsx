...
export default async function LoanDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (!loan) return <div style={{ padding: 40 }}>Loan not found</div>;
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                    <div className={s.card}>
                        ...
                        <div className={s.kpiRow} style={{ gridTemplateColumns: 'repeat(2, 1fr)' }}>
                            ...
                            <div className={s.kpiCard}><div className={s.kpiLabel}>Funded Date</div><div className={s.kpiValue}>{new Date(loan.fundedDate).toLocaleDateString()}</div></div>
                            <div className={s.kpiCard}><div className={s.kpiLabel}>Maturity Date</div><div className={s.kpiValue}>{new Date(loan.maturityDate).toLocaleDateString()}</div></div>
                        </div>
                    </div>

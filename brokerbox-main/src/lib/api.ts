...
/** Handle Supabase/Postgres and generic DB errors. */
export function handleDbError(err: unknown): NextResponse {
  const msg = err instanceof Error ? err.message : 'Database error';
  if (msg.includes('Unique constraint') || msg.includes('duplicate key') || msg.includes('23505')) {
    return NextResponse.json({ error: 'A record with this value already exists.' }, { status: 409 });
  }
  if (msg.includes('Record to update not found') || msg.includes('P2025') || msg.includes('0 rows')) {
    return NextResponse.json({ error: 'Not found.' }, { status: 404 });
  }
  if (msg.includes('Foreign key') || msg.includes('P2003') || msg.includes('23503')) {
...
/** @deprecated Use handleDbError */
export const handlePrismaError = handleDbError;
